variable "data_protection_backup_vault_lookup" {
  description = <<EOT
Map of data_protection_backup_vault_lookup, attributes below
Required:
    - name
    - resource_group_name
EOT
  ...
  validation {
    condition = alltrue([
      for k, v in var.data_protection_backup_vault_lookup : (
